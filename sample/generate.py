...
def main():
    ...
    max_frames = 196 if args.dataset in ['kit', 'humanml'] else 60   #wlasl30 :60
    fps = 12.5 if args.dataset == 'kit' else 20
    n_frames = min(max_frames, int(args.motion_length*fps))
    is_using_data = not any([args.input_text, args.text_prompt, args.action_file, args.action_name])  #True
    ...
    for rep_i in range(args.num_repetitions):
        ...
        sample = model.rot2xyz(x=sample, mask=rot2xyz_mask, pose_rep=rot2xyz_pose_rep, glob=True, translation=False,    # translation = False
                               jointstype='smplx', vertstrans=True, betas=None, beta=0, glob_rot=None,
        ...
        all_poserot.append(poserot.cpu().numpy())
        all_motions.append(sample.cpu().numpy())
        all_lengths.append(model_kwargs['y']['lengths'].cpu().numpy())

        print(f"created {len(all_motions) * args.batch_size} samples")
    
    all_poserot = np.concatenate(all_poserot, axis=0)  #(15, 53, 6, 60)
    all_poserot = all_poserot[:total_num_samples]  # [bs, njoints, 6, seqlen]
    ...
    #save pose rotation
    pose_path = os.path.join(out_path, 'pose_results.npy')  
    print(f"saving results file to [{pose_path}]")
    np.save(pose_path ,
            {'motion': all_poserot, 'text': all_text, 'lengths': all_lengths,
    ...
    elif args.dataset == 'wlasl30':
        skeleton = paramUtil.asl_kinematic_chain      # need to add hands
    else: 
        skeleton = paramUtil.asl_kinematic_chain 

 

    hands = paramUtil.t2m_hands_chain
    # st()
    ...
    for sample_i in range(args.num_samples):
        ...
        for rep_i in range(args.num_repetitions):
            ...
            motion = all_motions[rep_i*args.batch_size + sample_i].transpose(2, 0, 1)[:length]  #(60, 127, 3)
