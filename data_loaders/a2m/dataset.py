...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, num_frames=1, sampling="conseq", sampling_step=1, split="train",
                 pose_rep="smplx", translation=True, glob=True, max_len=-1, min_len=-1, num_seq_max=-1, **kwargs):
        ...
        if  self.dataname == "wlasl30" or "wlasl_smplx100":
    ...
    def _get_item_data_index(self, data_index):
        ...
        inp, action = self.get_pose_data(data_index, frame_ix)  # action 是 0-40的数字 inp 是 25*6*60  inp:  torch.Size([53, 6, 60])  input sequence 
        
        # adapat to our dataset wlasl30
        if (self.dataname == "wlasl30" ) : 
            action_name = self._label_to_action[str(action)] 
            description = self._action2description[action_name]           
            # output = {'inp': inp, 'action': action, 'action_text': action_name}  # modify to text here
            output = {'inp': inp, 'action': action, 'text': action_name, 'description': description} 
            return output
        # adapat to our dataset wlasl_smplx100
        if (self.dataname == "wlasl_smplx100" ) : 
            action_name = self._label_to_action[str(action)] 
            # description = self._action2description[action_name]           
            # output = {'inp': inp, 'action': action, 'action_text': action_name}  # modify to text here
            output = {'inp': inp, 'action': action, 'text': action_name} 
            # output = {'inp': inp, 'action': action, 'text': action_name, 'description': description} 
            return output
