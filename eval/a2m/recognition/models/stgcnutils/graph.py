...
from utils.config import SMPL_KINTREE_PATH, SMPLX_KINTREE_PATH
...
class Graph:
    ...
    def __init__(self,
                 layout='openpose',
                 strategy='uniform',
                 kintree_path=SMPLX_KINTREE_PATH,
                 max_hop=1,
                 dilation=1):
    ...
    def get_edge(self, layout):
        if layout == 'openpose':
            ...
            use_skeleton=False
            if use_skeleton:
                self.num_node = 55 # need to change 55 if use skeleton 53-->55
            else:
                self.num_node = 53 # pose rotation
            self_link = [(i, i) for i in range(self.num_node)]
