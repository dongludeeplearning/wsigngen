...
def evaluate(args, model, diffusion, data):
    # for wlasl30 smplx version   modify to wlasl103
    torch.multiprocessing.set_sharing_strategy('file_system')

    bs = args.batch_size
    args.num_classes = 103  # 30  to 103
    args.nfeats = 6
    args.njoint = 53
    args.cond_mode = 'text'
    ...
    recogparameters["pose_rep"] = "smplx"
    ...
    # Done can modify to train and test
    load_seperate_flag = True 
    if load_seperate_flag:
        
        datasetGT = {'train': [data['train']], 'test': [data['test']]} 
    else:
...
def evaluate_old(args, model, diffusion, data):
    torch.multiprocessing.set_sharing_strategy('file_system')
